...
typedef struct {
    ...
    XCircular_shift_reg_uio_map maps[ MAX_UIO_MAPS ];
} XCircular_shift_reg_uio_info;

/***************** Variable Definitions **************************************/
static XCircular_shift_reg_uio_info uio_info;
...
static int uio_info_read_name(XCircular_shift_reg_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCircular_shift_reg_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCircular_shift_reg_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCircular_shift_reg_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCircular_shift_reg_Initialize(XCircular_shift_reg *InstancePtr, const char* InstanceName) {
	XCircular_shift_reg_uio_info *InfoPtr = &uio_info;
