...
// wrapc file define:
#define AUTOTB_TVIN_dout "../tv/cdatafile/c.window_avg.autotvin_dout.dat"
#define AUTOTB_TVOUT_dout "../tv/cdatafile/c.window_avg.autotvout_dout.dat"
...
// tvout file define:
#define AUTOTB_TVOUT_PC_dout "../tv/rtldatafile/rtl.window_avg.autotvout_dout.dat"
...
static bool RTLOutputCheckAndReplacement(std::string &AESL_token, std::string PortName) {
  ...
  return err;}
extern "C" void window_avg_hw_stub_wrapper(volatile void *, volatile void *);
...
{
transaction<8> tr(819200);
aesl_fh.read(AUTOTB_TVOUT_PC_gmem, tr.p, tr.tbytes);
tr.reorder();
tr.send<1>((char*)__xlx_apatb_param_din, 409600);
...
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<8> > gmem_pc_buffer(819200);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 409600; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_din)[j*1+0] = gmem_pc_buffer[i].range(7, 0).to_int64();
}
            for (int j = 0, e = 409600; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_dout)[j*1+0] = gmem_pc_buffer[i].range(7, 0).to_int64();
}}}
        } // end transaction
      } // end file is good
...
tr.import<1>((char*)__xlx_apatb_param_din, 409600, 0);
  ...
  __xlx_offset_byte_param_dout = 409600*1;
...
aesl_fh.write(AUTOTB_TVIN_gmem, tr.p, tr.tbytes);
  ...
  tcl_file.set_num(819200, &tcl_file.gmem_depth);
#else
