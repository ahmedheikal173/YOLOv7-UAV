...
typedef struct {
    ...
    XWindow_avg_uio_map maps[ MAX_UIO_MAPS ];
} XWindow_avg_uio_info;

/***************** Variable Definitions **************************************/
static XWindow_avg_uio_info uio_info;
...
static int uio_info_read_name(XWindow_avg_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XWindow_avg_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XWindow_avg_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XWindow_avg_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XWindow_avg_Initialize(XWindow_avg *InstancePtr, const char* InstanceName) {
	XWindow_avg_uio_info *InfoPtr = &uio_info;
