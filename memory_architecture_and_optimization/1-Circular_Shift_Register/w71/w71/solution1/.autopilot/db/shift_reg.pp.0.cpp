...
void shift_reg(dType din, dType dout[4]);
__attribute__((sdx_kernel("circular_shift_reg", 0))) void circular_shift_reg(dType din, dType dout[4]);
...
#pragma HLS RESOURCE variable=mem core=RAM_1P_BRAM
 ...
 T dummy;
    wptr = 0;
    rptr = 0;
    VITIS_LOOP_23_1: for (int i = 0; i < N; i++) mem[i] = dummy;
  }
...
__attribute__((sdx_kernel("circular_shift_reg", 0))) void circular_shift_reg(dType din, dType dout[4]) {_ssdm_SpecArrayDimSize(dout, 4);
#pragma HLSDIRECTIVE TOP name=circular_shift_reg
# 23 "../src/shift_reg.cpp"


  static circular_shift<dType, 4> regs;
