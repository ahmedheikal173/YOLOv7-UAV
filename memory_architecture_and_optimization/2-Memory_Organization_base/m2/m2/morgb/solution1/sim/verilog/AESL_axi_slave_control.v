...
//------------------------Parameter----------------------
`define TV_IN_load "../tv/cdatafile/c.interleave_manual_seq.autotvin_load.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter load_DEPTH = 1;
reg [31 : 0] load_OPERATE_DEPTH = 0;
parameter load_c_bitwidth = 1;
parameter START_ADDR = 0;
parameter interleave_manual_seq_continue_addr = 0;
parameter interleave_manual_seq_auto_start_addr = 0;
parameter load_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_load [load_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_load [ (load_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * load_DEPTH -1 : 0] = '{default : 'hz};
reg load_write_data_finish;
...
//write load reg
reg [31 : 0] write_load_count = 0;
reg write_load_run_flag = 0;
reg write_one_load_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & load_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        load_write_data_finish <= 0;
        write_load_run_flag <= 0; 
        write_load_count = 0;
        count_operate_depth_by_bitwidth_and_depth (load_c_bitwidth, load_DEPTH, load_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_load_run_flag <= 1; 
            write_load_count = 0;
        end
        if (write_one_load_data_done === 1) begin
            write_load_count = write_load_count + 1;
            if (write_load_count == load_OPERATE_DEPTH) begin
                write_load_run_flag <= 0; 
                load_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_load
    integer write_load_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] load_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = load_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_load_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write load data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (load_c_bitwidth < 32) begin
                        load_data_tmp_reg = mem_load[write_load_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < load_c_bitwidth) begin
                                load_data_tmp_reg[j] = mem_load[write_load_count][i*32 + j];
                            end
                            else begin
                                load_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_load[write_load_count * four_byte_num  + i]!==load_data_tmp_reg) begin
                    write (load_data_in_addr + write_load_count * four_byte_num * 4 + i * 4, load_data_tmp_reg, write_load_resp);
                    image_mem_load[write_load_count * four_byte_num + i]=load_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_load_data_done <= 1;
                @(posedge clk);
                write_one_load_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_load_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [load_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (load_c_bitwidth , factor);
  fp = $fopen(`TV_IN_load ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_load); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < load_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_load [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
