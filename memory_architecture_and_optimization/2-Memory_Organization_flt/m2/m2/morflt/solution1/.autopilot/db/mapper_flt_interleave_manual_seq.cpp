...
extern "C" void flt_interleave_manual_seq(char*, short*, char);
extern "C" void apatb_flt_interleave_manual_seq_hw(volatile void * __xlx_apatb_param_x_in, volatile void * __xlx_apatb_param_y, char __xlx_apatb_param_load) {
  ...
  for (int j = 0, e = 396; j != e; ++j) {
    ...
    _xlx_tmp_sc.range(7, 0) = ((char*)__xlx_apatb_param_y)[j*2+0];
    _xlx_tmp_sc.range(15, 8) = ((char*)__xlx_apatb_param_y)[j*2+1];
    __xlx_y__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_y = 396;
  ...
  // DUT call
  flt_interleave_manual_seq(__xlx_x_in__input_buffer, __xlx_y__input_buffer, __xlx_apatb_param_load);
...
// print __xlx_apatb_param_y
  ...
  for (int i = 0; i < __xlx_size_param_y; ++i) {
    ((char*)__xlx_apatb_param_y)[i*2+0] = __xlx_y_output_buffer[i].range(7, 0).to_uint();
    ((char*)__xlx_apatb_param_y)[i*2+1] = __xlx_y_output_buffer[i].range(15, 8).to_uint();
  }
}
