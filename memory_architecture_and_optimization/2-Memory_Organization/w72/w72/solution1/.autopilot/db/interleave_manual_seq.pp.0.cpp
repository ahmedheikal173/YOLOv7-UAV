...
void interleave(ap_int<8> x_in[9], ap_int<8> y[9 / 3],
                bool load);
void interleave_manual_rnd(ap_int<8> x_in[9], ap_int<8> y[9 / 3],
                       bool load);
...
#pragma HLS RESOURCE variable = x2 core = RAM_1P_BRAM
 ...
 idx = 0;
  ...
  void write_seq(ap_uint<(16)> i, T x_in[N]);
  T read_seq(ap_uint<(16)> i, int offset);
};
...
template <typename T, int N>
T interleave_mem_seq<T, N>::read_seq(ap_uint<(16)> i,
                                     int offset) {
  T tmp = 0;
  switch (offset) {
    case 0:
...
template <typename T, int N>
void interleave_mem_seq<T, N>::write_seq(ap_uint<(16)> i,
                                         T x_in[N]) {
  int tmp = x_in[i];
  switch (sel++) {
    case 0:
...
void interleave_manual_seq(ap_int<8> x_in[9],
                           ap_int<8> y[9 / 3], bool load) {
#pragma HLS RESOURCE variable=x_in core=RAM_1P_BRAM
#pragma HLS RESOURCE variable=y core=RAM_1P_BRAM

 static interleave_mem_seq<ap_int<8>, 9> x;
  int idx = 0;
