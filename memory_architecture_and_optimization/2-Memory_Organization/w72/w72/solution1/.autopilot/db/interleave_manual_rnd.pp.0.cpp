...
void interleave(ap_int<8> x_in[9], ap_int<8> y[9 / 3],
                bool load);
__attribute__((sdx_kernel("interleave_manual_rnd", 0))) void interleave_manual_rnd(ap_int<8> x_in[9], ap_int<8> y[9 / 3],
                       bool load);
void interleave_manual_seq(ap_int<8> x_in[9], ap_int<8> y[9 / 3],
                       bool load);
...
#pragma HLS RESOURCE variable=x2 core=RAM_1P_BRAM
  ...
  void write_rnd(ap_uint<(16)> i, T x_in[N]);
  T read_rnd(ap_uint<(16)> i, int offset);
};
...
template <typename T, int N>
T interleave_mem_rnd<T, N>::read_rnd(ap_uint<(16)> i,
                             int offset) {
...
template <typename T, int N>
void interleave_mem_rnd<T, N>::write_rnd(ap_uint<(16)> i,
                                 T x_in[N]) {
  T tmp = x_in[i];
  switch (i % 3) {
    case 0:
...
__attribute__((sdx_kernel("interleave_manual_rnd", 0))) void interleave_manual_rnd(ap_int<8> x_in[9],
                           ap_int<8> y[9 / 3], bool load) {_ssdm_SpecArrayDimSize(x_in, 9);_ssdm_SpecArrayDimSize(y, 3);
...
#pragma HLS RESOURCE variable=y core=RAM_1P_BRAM
 ...
 static interleave_mem_rnd<ap_int<8>, 9> x;
  int idx = 0;
